...
class FrontCompartment(@OneToOne @MapsId("compartmentID") val compartment: Compartment
    ...
    companion object {
        val layerSort = Sort.by("${FrontCompartment::compartment.name}.${Compartment::layer.name}.${Layer::number.name}")
        val numberSort = Sort.by("${FrontCompartment::compartment.name}.${Compartment::number.name}")

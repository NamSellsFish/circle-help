...
@Service
class Logic(@Autowired private val shelvesRepository: ShelvesRepository,
    ...
    fun productNotFoundResponse(id: Long) : ErrorResponse {
        return ErrorResponse("No product with id: $id", HttpStatus.NOT_FOUND)
    }
    ...
    fun expiredProductArrangementAttemptResponse(packageProduct: PackageProduct) : ErrorResponse {
        return ErrorResponse(
            "Attempted to arrange expired or expiring product:\n" +
                    "   Product ID: ${packageProduct.product.id}\n" +
